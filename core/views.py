...
def manual_payment(request, reference):
    ...
    try:
        ...
        return render(
            request, "manual_payment.html", {"amount": amount, "reference": reference}
        )
    except Transaction.DoesNotExist:
        return JsonResponse(
            {"success": False, "errors": {"general": "Transaction not found"}}
        )
...
def initiate_manual_payment(request, amount_in_kobo):
    ...
    if amount_in_naira < Decimal("1000"):
        return JsonResponse(
            {"success": False, "errors": {"amount": "Minimum amount is ₦1,000"}}
        )
        ...
        return JsonResponse(
            {
                ...
                "redirect_url": reverse(
                    "manual_payment", kwargs={"reference": reference}
                ),
            }
        )
        ...
        return JsonResponse(
            {
                "success": False,
                "errors": {
                    "general": "An error occurred while initializing manual payment. Please try again."
                },
            }
        )
...
def confirm_manual_payment(request):
    try:
        ...
        if not cached_user_id or cached_user_id != request.user.id:
            return JsonResponse(
                {"success": False, "error": "Invalid or expired transaction reference"}
            )

        try:
            transaction = Transaction.objects.get(
                payment_reference=reference, wallet__user=request.user, status="pending"
            )
        except Transaction.DoesNotExist:
            return JsonResponse(
                {
        ...
        try:
            ...
            return JsonResponse({"success": True, "redirect_url": "/add-funds/"})
        except Exception as e:
...
def login_view(request):
    ...
    if user is not None:
        ...
        return JsonResponse(
            {"success": False, "errors": {"general": "Invalid credentials"}}
        )
...
def reset_password(request):
    try:
        ...
        if not stored_otp or stored_otp != otp:
            return JsonResponse(
                {"success": False, "errors": {"otp": "Invalid or expired OTP"}}
            )
...
def profile(request):
    total_spent = (
        Transaction.objects.filter(
            wallet=request.user.wallet, status="success"
...
def initiate_payment(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if Order.objects.filter(transaction=transaction).exists():
                return redirect(
                    "marketplace:after_checkout", order_id=transaction.order.id
                )
            return redirect("add_funds")
        except Transaction.DoesNotExist:
            return redirect("add_funds")
    ...
    try:
        ...
        if (
            amount < gateway_config["min_amount"]
            or amount > gateway_config["max_amount"]
        ):
            return JsonResponse(
                {
                    "success": False,
                    "errors": {
                        "amount": f'Amount must be between ₦{gateway_config["min_amount"]:,.2f} and ₦{gateway_config["max_amount"]:,.2f}'
                    },
                }
            )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if (
                ...
                and response_data["data"]["checkout_url"]
            ):
                transaction = Transaction.objects.create(
                    wallet=request.user.wallet,
                    amount=amount_in_naira,
